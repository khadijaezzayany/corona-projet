...
const previousBut = document.querySelector('.previous');
...
startBut.addEventListener('click', startTest);

answerInputs.addEventListener('change', (event) => {
...
function hidepreviousBut() {
    if (currentQuestionIndex === 0) {
        previousBut.classList.add('hide');
    } else {
        previousBut.classList.remove('hide');
    }
}
...
nextBut.addEventListener('click', () => {
    if (currentQuestionIndex < 21) {
        ...
        nextBut.disabled = true;
        if (currentQuestionIndex === 21) {
            nextBut.innerText = 'Terminer le test';
            nextBut.style.width = '161px';
            nextBut.classList.add('result')
...
previousBut.addEventListener('click', () => {
    ...
    if (currentQuestionIndex === 21) {
        ...
        nextBut.innerText = 'Suivant';
        nextBut.classList.remove('result')
...
function showResult(severity) {
    ...
    startBut.textContent = ' Recommencer le test'
    startBut.style.width = '280px'
    ...
    if (severity === 0) {
        ...
        resultMessage[1].style.color = '#1078AD'
        resultMessage[0].style.fontSize = '18px'
